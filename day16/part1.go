...
func (r *Room) shootLaser(startX, startY int) {
	var lasers []*Laser

	// map[index]map[direction]bool
	seen := map[int]map[int]bool{}
	for i := range r.grid {
		seen[i] = map[int]bool{}
	}

	nextDirections := laserMovement[r.grid[startY*r.width+startX].kind][DIRECTION_RIGHT]
